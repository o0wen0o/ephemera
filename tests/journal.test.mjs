...
const journalOf = ({ entries = [], catalog = { tags: [], moods: [] }, ...sync }) => ({
    entries,
    catalog,
...
const fixture = () => ({
    entries: [
        ...
        { ...structuredClone(entry), id: "unrelated", tags: ["散步"], mood: "平静" }
    ],
    catalog: { tags: ["阅读", "日常", "散步", "尚未使用"], moods: ["开心", "平静"] },
    sync: defaultSync()
});

test("reads original array and version 1 export without changing diary data", () => {
    for (const old of [[entry], { app: "芸窗", version: 1, entries: [entry] }])
        assert.deepEqual(parseJournal(JSON.stringify(old)).entries, [entry]);
});
test("new snapshot round trip preserves unused custom choices and empty mood catalogs", () => {
    const value = {
        entries: [entry],
        catalog: { tags: ["未使用"], moods: [] },
        sync: { dirtyIds: ["test-only"], tombstones: [], bases: {} }
    };
...
test("duplicate entry ids collapse to the most recently updated copy", () => {
    ...
    const parsed = parseJournal(
        JSON.stringify({ entries: [entry, newer, entry], catalog: { tags: [], moods: [] } })
    );
...
test("a queued local deletion preserves cloud content and uses the last seen version as its write condition", () => {
    ...
    const source = journalOf({
        dirtyIds: [entry.id],
        tombstones: [{ entry, deleted_at: deletedAt }],
        bases: { [entry.id]: entry.updated_at }
    });
    const plan = planSync(source, [{ ...entry, deleted_at: null }]);
    assert.equal(plan.uploads[0].row.id, entry.id);
    for (const key of ["title", "body", "mood", "weather", "tags", "favorite"])
        assert.deepEqual(plan.uploads[0].row[key], entry[key]);
...
test("empty trash stays local across reload and sync, and retains pending cloud content", () => {
    ...
    assert.equal(
        planSync(
            uploaded.journal,
            uploaded.uploads.map((u) => u.row)
        ).journal.sync.tombstones.length,
        0
    );
