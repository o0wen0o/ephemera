    ...
    watcher ||= new IntersectionObserver(
        (seen) => {
            for (const { target, isIntersecting } of seen)
                if (isIntersecting) watched.get(target)?.();
        },
        { rootMargin: "250px" }
    );
...
function Viewer({ url, onClose }: { url: string; onClose: () => void }) {
    return (
        <Modal label="查看图片" onClose={onClose} wide>
            <div className="photo-viewer">
                <button type="button" className="text-btn photo-viewer-close" onClick={onClose}>
                    <X size={18} />
                    关闭图片
                </button>
...
function PhotoButton({
    ...
    return (
        <button
            ref={box}
            type="button"
            className={compact ? "photo-thumb" : "photo-preview"}
            onClick={() => (url ? onOpen(url) : again())}
            aria-label={url ? "放大日记照片" : error || "图片加载中"}
        >
            {url ? (
                <img src={url} alt="" />
            ) : compact ? null : (
    ...
    return (
        <>
            <span className="photo-strip">
                {paths.slice(0, MAX_PHOTOS).map((path) => (
                    <PhotoButton
                        key={path}
                        path={path}
                        userId={userId}
...
export function Photos({
    ...
    const add = async (file?: File) => {
        ...
        try {
            ...
            if (active.current) setError(e instanceof Error ? e.message : "图片上传失败，请重试。");
        } finally {
    ...
    return (
        <section className="journal-photos" aria-label="日记图片">
            {onChange && (
                <div className="photo-heading">
                    <span>
                        日常留影 · {paths.length} / {MAX_PHOTOS}
                    </span>
                    <Help label="图片">
                        登录并联网后可添加最多 3
                        张照片，会自动压缩。备份文件不含图片。上传时请勿关闭页面。
                    </Help>
                    <button
                        type="button"
                        className="text-btn"
                        disabled={!userId || uploading || paths.length >= MAX_PHOTOS}
