...
export function Editor({ entry, tags, moods, onClose, onSave }: {
  ...
  const addTag = (raw: string) => {
    ...
    if (!name) return;
    if (!validName(name)) { setError(`标签请使用 1–${NAME_LIMIT} 个字，不含逗号。`); return; }
  ...
  const suggestions = tags.filter(t => !value.tags.includes(t) && t.toLocaleLowerCase().includes(needle));
  // Keep showing a mood the entry still carries after it was removed from the catalog.
  const availableMoods = value.mood && !moods.includes(value.mood) ? [...moods, value.mood] : moods;
  return <section className="editor writing-sheet" aria-label="写日记">
    ...
    <div className="editor-paper"><div className="writing-date"><DatePicker value={value.date} onChange={date => update({ date })}/></div>
      ...
      <div className="writing-details">
        ...
        <fieldset><legend>此刻心情 <Help label="心情">可不选，再次点击取消；在整理书页中管理心情。</Help></legend><div className="choice-chips">{availableMoods.map(m => <button type="button" key={m} aria-pressed={value.mood === m} onClick={() => update({ mood: value.mood === m ? '' : m })}><span aria-hidden="true">{moodGlyph(m)}</span>{m}</button>)}</div></fieldset>
    ...
    <footer className="editor-footer"><span>{countWords(value.body)} 字</span><button type="button" className="primary" onClick={submit}><Save size={16}/>保存日记</button>{error && <p className="field-error" role="alert">{error}</p>}</footer>
  </section>;
}
