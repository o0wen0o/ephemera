...
export function Calendar({
    ...
    const commitYear = () => {
        ...
        if (Number.isInteger(next) && next >= 1900 && next <= 9999)
            onMonthChange(monthStart(next, monthIndex));
        setYearDraft(null);
    };
    ...
    const moveDay = (event: KeyboardEvent, date: string) => {
        ...
        if (next.getMonth() !== monthIndex || next.getFullYear() !== year)
            onMonthChange(monthStart(next.getFullYear(), next.getMonth()));
    };
    ...
    return (
        <section
            ref={root}
            className={`journal-calendar${compact ? " compact" : ""}`}
            aria-label={compact ? "选择日记日期" : "日历回顾"}
        >
            <div className="cal-toolbar">
                ...
                >
                    {year} 年 {monthIndex + 1} 月 <ChevronDown size={15} />
                </button>
                <div>
                    <button
                        type="button"
                    ...
                    <div className="month-options">
                        {Array.from({ length: 12 }, (_, i) => (
                            <button
                                ...
                                onClick={() => {
                                    onMonthChange(monthStart(year, i));
                ...
                <>
                    ...
                    <div className="cal-days">
                        {Array.from({ length: Math.ceil((offset + days) / 7) * 7 }, (_, i) => {
                            ...
                            if (day < 1 || day > days)
                                return <span key={i} className="cal-blank" />;
                            ...
                            return (
                                <button
                                    ...
                                    key={i}
                                    data-date={date}
                                    tabIndex={tabDate === date ? 0 : -1}
                                    onFocus={() => setFocusDay(date)}
                                    onKeyDown={(e) => moveDay(e, date)}
                                    aria-label={`${date}${mark ? `，${mark.count} 篇日记` : ""}`}
                                    aria-pressed={value === date}
                                    aria-current={date === today ? "date" : undefined}
                                    className={`cal-day${mark ? " has-record" : ""}`}
                                    onClick={() => onChange(date)}
                                >
                                    <span>{day}</span>
                                    {mark && <i />}
                                    {!compact && mark && <small>{mark.title}</small>}
                                </button>
                            );
                        })}
                    </div>
            ...
            <div className="cal-footer">
                <Help label="日历">
                    {compact ? "方向键移动，回车选择日期。" : "圆点标记有日记的日子。"}
                </Help>
                <button
                    type="button"
                    onClick={() => {
...
export function DatePicker({
    ...
    useEffect(() => {
        ...
        root.current
            ?.querySelector<HTMLButtonElement>('.cal-day[tabindex="0"]')
            ?.focus({ preventScroll: true });
    ...
    return (
        ...
        >
            ...
            {open && (
                <div id={id} className="date-popover">
                    <Calendar
                        compact
                        month={month}
