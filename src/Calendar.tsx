...
export function Calendar({ value, onChange, month, onMonthChange, entries = [], compact = false }: {
  ...
  const commitYear = () => {
    ...
    if (Number.isInteger(next) && next >= 1900 && next <= 9999) onMonthChange(new Date(next, monthIndex, 1, 12));
  ...
  const moveDay = (event: KeyboardEvent, date: string) => {
    ...
    if (next.getMonth() !== monthIndex || next.getFullYear() !== year) onMonthChange(new Date(next.getFullYear(), next.getMonth(), 1, 12));
  };
  const tabDate = focusDay.startsWith(localDate(first).slice(0, 7)) ? focusDay : localDate(first);
  return <section ref={root} className={`journal-calendar${compact ? ' compact' : ''}`} aria-label={compact ? '选择日记日期' : '日历回顾'}>
    ...
    {chooseMonth ? <div className="month-picker">
      <div className="year-picker"><button type="button" className="icon-btn" aria-label="上一年" onClick={() => moveMonth(-12)}><ChevronLeft size={17}/></button><label>年份 <input aria-label="年份" type="number" min="1900" max="9999" value={yearInput} onChange={e => setYearInput(e.target.value)} onBlur={commitYear} onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); commitYear(); } }}/></label><button type="button" className="icon-btn" aria-label="下一年" onClick={() => moveMonth(12)}><ChevronRight size={17}/></button></div>
      <div className="month-options">{Array.from({ length: 12 }, (_, i) => <button type="button" key={i} aria-pressed={i === monthIndex} onClick={() => { onMonthChange(new Date(year, i, 1, 12)); setChooseMonth(false); }}>{i + 1} 月</button>)}</div>
    </div> : <>
      ...
      <div className="cal-days">{Array.from({ length: Math.ceil((offset + days) / 7) * 7 }, (_, i) => {
        ...
        const date = localDate(new Date(year, monthIndex, day, 12)); const records = counts.get(date) || [];
