...
export function planSync(source: Journal, cloudRows: CloudEntry[], now = new Date().toISOString(), makeId = () => crypto.randomUUID()): SyncPlan {
  ...
  for (const id of new Set([...local.keys(), ...tombstones.keys(), ...remote.keys()])) {
    ...
    const cloudChanged = !!cloud && (base === undefined || Date.parse(stamp(cloud)) !== Date.parse(base));
    if (dirty.has(id)) {
      if (deleted) {
 ...
 */
export function syncSignature(userId: string, pulse: number, journal: { entries: Entry[]; sync: SyncMeta }) {
